<script>
    ...
    async function deleteData(name, year) {
        const res = await fetch("/api/v2/esco/" + name + "/" + year, {
            method: "DELETE"
        }).then(function (res) {
            visible = true;
            getData();      
	...
	async function BorrarEsco(countryDelete, yearDelete){
        ...
        const res = await fetch("/api/v2/esco/"+countryDelete+"/"+yearDelete,
			{
				method: "DELETE"
			}).then(function (res){
				getesco();
	...
	async function Loadesco(){
        ...
        const res = await fetch("/api/v2/esco/loadInitialData",
			{
				method: "GET"
			}).then(function (res){
				getesco();
...
<Button id ="siguientebtn" on:click="{getNextPage}">
		...
		<tbody>
				<tr>
					...
					<td><Button 
						outline
						color= "primary"
						on:click={insertesco}>insert</Button>
					</td>
				</tr>
				{#each esco as Esco}
				<tr>
					<td><a href="#/escoTable/{Esco.country}">{Esco.country}</a></td>
					<td>{Esco.year}</td>
					<td>{Esco.esco_tot}</td>
					<td>{Esco.esco_men}</td>
					<td>{Esco.esco_wom}</td>
					...
					<td><Button outline color="danger" on:click={BorrarEsco(Esco.country,Esco.year)}>
						Borrar
					</Button>
				</tr>
				{/each}
	...
	<div align="center">
		<Button outline color="success" on:click={Loadesco}>
			Cargar datos
		</Button>
		<Button outline color="danger" on:click={Borraresco}>
			Borrar todo
		</Button>
	</div>
{/await}
